  ...
  const deleteItem = useCallback((event: React.UIEvent, item: ChatHistoryItem) => {
    ...
    if (db) {
      deleteById(db, item.id)
        ...
        .catch((error) => {
          toast.error('Falha ao deletar chat');
          logger.error(error);
        });
    }
  }, []);
        ...
        <div className="flex-1 flex flex-col h-full w-full overflow-hidden">
          <div className="p-4 space-y-3">
            ...
            >
              ...
              <span className="text-sm font-medium">Inicie um novo chat</span>
            </a>
          ...
          <div className="text-gray-600 dark:text-gray-400 text-sm font-medium px-4 py-2">Seus Chats</div>
          <div className="flex-1 overflow-auto px-3 pb-3">
            {filteredList.length === 0 && (
              <div className="px-4 text-gray-500 dark:text-gray-400 text-sm">
                {list.length === 0 ? 'Sem Chats anteriores' : 'Nenhum resultado encontrado'}
              </div>
            )}
                ...
                {dialogContent?.type === 'delete' && (
                  <>
                    <div className="p-6 bg-white dark:bg-gray-950">
                      <DialogTitle className="text-gray-900 dark:text-white">Deletar Chat?</DialogTitle>
                      <DialogDescription className="mt-2 text-gray-600 dark:text-gray-400">
                        <p>
                          Você está preste apagar{' '}
                          <span className="font-medium text-gray-900 dark:text-white">
                            {dialogContent.item.description}
                          </span>
                        </p>
                        <p className="mt-2">Tem certeza que deseja deletar?</p>
                      </DialogDescription>
                    </div>
